...
const bgMain = 'bg-telegram-main';
const bgContent = 'bg-telegram-surface';
const headerBg = 'bg-telegram-main';
const textPrimary = 'text-text-main';
const textSecondary = 'text-text-muted';
const accent = 'text-telegram-accent';
...
const ProfileView = memo(() => (
  <MockupScreen className={bgMain}>
    <div className="absolute left-0 right-0 top-0 z-30 flex items-center justify-between p-3 text-text-main drop-shadow-md">
      <LuArrowLeft size={24} />
    ...
    <div className="relative aspect-square w-full shrink-0 overflow-hidden bg-black">
      ...
      <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-telegram-main to-transparent"></div>

      <div className="absolute bottom-4 left-5 z-20 text-text-main">
    ...
    <MockupContent className="space-y-6 p-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="font-medium text-telegram-accent">On</div>
      </div>
    </MockupContent>
...
const ChatView = memo(() => (
  <MockupScreen className={bgContent}>
    <div className={clsx('z-20 flex h-[56px] shrink-0 items-center px-2 shadow-sm', headerBg)}>
      <div className="cursor-pointer rounded-full p-2 hover:bg-slate-800/50">
    ...
    <MockupContent className="z-10 space-y-2 p-3 pb-4">
      <div className="my-4 flex justify-center">
        <div className="rounded-full border border-white/5 bg-telegram-main/80 px-3 py-1 text-xs font-bold text-text-main shadow-sm backdrop-blur-sm">
          Today
        </div>
      </div>

      <div className="ml-auto flex max-w-[85%] justify-end">
        <div className="relative rounded-bl-xl rounded-tl-xl rounded-tr-xl rounded-br-none bg-blue-600/50 p-2.5 text-text-main shadow-sm">
          <p className="text-[15px] leading-snug">Hey! Nice new profile picture! 🔥</p>
          <div className="mt-1 flex items-center justify-end gap-1 text-blue-300">
            <span className="text-[11px]">10:41 AM</span>
            <LuCheck size={14} className="text-telegram-accent" />
          </div>
        </div>
      </div>

      <div className="flex max-w-[85%] justify-start">
        <div className="relative rounded-bl-none rounded-br-xl rounded-tl-xl rounded-tr-xl bg-slate-800 p-2.5 text-text-main shadow-sm">
    ...
    <Navigation className={headerBg} safeAreaClassName="pb-8">
      <div className="relative z-20 flex items-end gap-2 px-2 pt-2">
        <div className="shrink-0 cursor-pointer rounded-full p-2.5 text-slate-400 transition-colors hover:bg-slate-800/50">
          <LuPaperclip size={24} />
        </div>
        <div className="flex min-h-[44px] min-w-0 flex-1 items-center gap-2 rounded-2xl border border-black/10 bg-[#0e1621] px-3">
          <input
            type="text"
            placeholder="Message"
            className="min-w-0 flex-1 bg-transparent py-2 text-[16px] text-text-main outline-none placeholder-[#6c7883]"
          />
          <LuSmile size={24} className="shrink-0 cursor-pointer text-[#7f91a4] hover:text-[#dbdee1]" />
        </div>
        <div className="shrink-0 cursor-pointer rounded-full bg-blue-600/50 p-3 text-text-main shadow-md transition-colors hover:bg-blue-600/70">
          <LuMic size={24} />
        </div>
      </div>
    </Navigation>
